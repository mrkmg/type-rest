...
describe("Special Endpoints", () => {
    ...
    it("_fullPath", () => {
        expect(api._fullPath).toEqual(["https://api1.test-domain.local/api/v1"]);
        expect(api.sub1._fullPath).toEqual(["https://api1.test-domain.local/api/v1", "sub1"]);
        expect(api.sub1.sub2._fullPath).toEqual(["https://api1.test-domain.local/api/v1", "sub1", "sub2"]);
    });
});
